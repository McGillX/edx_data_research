...
def main():
    ...
    run_parser.add_argument('action', help='Run analytics based on argument',
                             nargs='?', default='basic')
    run_parser.add_argument('db_name',
                             help='Name of database where each database '
                             'corresponds to a course offering')
    run_parser.add_argument('-u', '--uri', help='URI to MongoDB database '
                            '(default: mongodb://localhost:27017)')
    run_parser.add_argument('-o', '--output', help='Path to directory to save '
                            'CSV report (defaults to current directory: '
                            '%(default)s)', default=os.getcwd(),
                            dest='output_directory')
    run_parser.add_argument('-j', '--json', help='Path to JSON file that may '
                            'be needed for some analytics commands')
    run_parser.add_argument('-c', '--csv', help='Path to CSV file that may be '
                            'needed for some analytics commands')
    run_parser.add_argument('-p', '--problem-id', help='Course specifc '
                            'problem ID that may be needed for some analytics '
                            'commands', dest='problem_id')
    run_parser.add_argument('-r', '--row-limit', help='Number of rows per CSV '
                            'file (default: %(default)s)', type=int,
                            default=100000, dest='row_limit')
    run_parser.add_argument('-a', '--anonymize', help='Only include hash id '
